...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* --- Responsive pricing grid & mobile tweaks --- */
    ...
    /* --- Simple carousel --- */
    .carousel{padding:0; overflow:hidden; position:relative;}
    .carousel .slides{position:relative; width:100%; height:0; padding-bottom:56.25%; border-radius:12px; overflow:hidden;}
    .carousel img{position:absolute; inset:0; width:100%; height:100%; object-fit:cover; opacity:0; transition:opacity .6s ease;}
    .carousel img.active{opacity:1;}
    .carousel .dots{position:absolute; left:50%; transform:translateX(-50%); bottom:12px; display:flex; gap:8px; z-index:2;}
    .carousel .dot{width:8px; height:8px; border-radius:999px; background:rgba(255,255,255,.45); border:1px solid rgba(0,0,0,.25); cursor:pointer}
    .carousel .dot.active{background:var(--brand); box-shadow:0 0 12px rgba(124,92,255,.8);}
    .carousel .caption{position:absolute; left:0; right:0; bottom:0; padding:10px 14px; background:linear-gradient(180deg,transparent,rgba(0,0,0,.55)); font-size:12px; color:#dfe6ff;}
    @media (max-width:640px){
      .carousel .slides{padding-bottom:62%;}
    }

  
/* --- Header base (mobile-friendly) --- */
.site-header{
  display:flex; align-items:center; justify-content:space-between;
  gap:12px; padding:10px 14px; margin:12px auto 18px;
  background: rgba(8,10,20,.55); border:1px solid rgba(255,255,255,.08);
  border-radius:14px; backdrop-filter: blur(8px);
}
.site-header .brand{
  display:inline-flex; align-items:center; gap:10px;
  font-weight:600; text-decoration:none; color:#e6e7f0;
  white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
  max-width: 60vw;
}
.brand-dot{
  width:10px; height:10px; border-radius:50%;
  background: radial-gradient(closest-side, #8b5cf6, #4f46e5);
  display:inline-block; box-shadow:0 0 14px rgba(139,92,246,.6);
}
.nav-links{ display:flex; align-items:center; gap:18px; }
.nav-links a{ color:#cfd3ff; text-decoration:none; font-weight:500; }
.nav-links a:hover{ text-decoration:underline; }
.hamburger{ display:none; width:40px; height:40px; border-radius:10px;
  border:1px solid rgba(255,255,255,.12); background: rgba(255,255,255,.04);
}
.hamburger span{ display:block; height:2px; margin:6px 9px;
  background:#e6e7f0; border-radius:2px;
}
@media (max-width: 640px){
  ...
  .nav-links{
    ...
    background: rgba(8,10,20,.92); border:1px solid rgba(255,255,255,.12);
    border-radius:14px; padding:12px; flex-direction:column; gap:10px;
    box-shadow: 0 18px 40px rgba(0,0,0,.5); z-index:1000;
  }
  .nav-links.open{ display:flex; }
  .nav-links a{ padding:12px 10px; border-radius:10px; background: rgba(255,255,255,.03); }
}
  ...
  <nav id="navMenu" class="nav-links">
    ...
    <section class="hero">
      <div class="card">
        ...
        <div class="card carousel" style="margin-top:14px;">
...
<div style="margin-top:8px;border-top:1px dashed rgba(255,255,255,.18);padding-top:10px">
  ...
  <div class="cta-row">
    ...
    <section class="card" id="support" style="margin-top:20px;">
      <h3>FAQ</h3>
...
confirmBooking = function(){
  ...
  (function(){
    const images = [
      {src:'/image/dental_1.png', alt:'Front desk at a local dental office'},
      {src:'/image/dental_2.png', alt:'Dentist and team at work in operatory'},
      {src:'/image/dental_3.png', alt:'Modern exam room and equipment'}
    ];
    ...
    let idx = 0, timer = null, lastX = null;
    ...
    // swipe on mobile
    slidesEl.addEventListener('touchstart', e => lastX = e.touches[0].clientX, {passive:true});
    ...
    slidesEl.addEventListener('touchend', ()=> lastX = null);
  })();
